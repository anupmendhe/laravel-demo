...
function loadStates(ref) {
    ...
    if(selected_country && selected_country!="" && selected_country!=0){
	 	  ...
	 	  $('select[id="state_id"]').find('option').remove().end().append('<option value="">-- Select State --</option>').val('');
     	$('select[id="city_id"]').find('option').remove().end().append('<option value="">-- Select City --</option>').val('');
        ...
        $.ajax({
              	...
              	beforeSend:function()
              	{
                  $('select[id="state_id"]').attr('readonly','readonly');
              	},
              	success:function(response)
              	{  
                  	if(response.status=="success")
                  	{
                      	$('select[id="state_id"]').removeAttr('readonly');
                      	...
                      	if(typeof(response.arr_state) == "object")
                      	{
                         	...
                         	$('select[id="state_id"]').html(option);
                      	}
                  	}
                  return false;
              	}	  
    ...
    if(selected_state && selected_state!="" && selected_state!=0)
    {
     ...
     $('select[id="city_id"]').find('option').remove().end().append('<option value="">-- Select City --</option>').val('');
          ...
          $.ajax({
             ...
             beforeSend:function()
              {
                  $('select[id="city_id"]').attr('readonly','readonly');
              },
              success:function(response)
              {
                  if(response.status=="success")
                  {
                      $('select[id="city_id"]').removeAttr('readonly');
